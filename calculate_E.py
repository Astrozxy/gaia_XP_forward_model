...
from model import load_h5, save_as_h5
...
def calculate_E(fn):
    ...
    all_distance = [ 1./np.clip(parallax+parallax_err*gen, a_min=0.0001, a_max=np.inf)
    ...
    all_position = [ SkyCoord(ra*units.deg,dec*units.deg,
                                  ...
                                  frame='icrs') for j, gen in enumerate(gaussian_generator)]
    ...
    with h5py.File('data/xp_bayestar_match/'+f'xp_reddening_match_{idx}.h5', 'w') as f:
        ...
        f['gdr3_source_id'] = d['gdr3_source_id'][:]
